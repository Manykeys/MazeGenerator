...
class MazeKruskalAlgoSpec extends AnyFlatSpec with Matchers {

  "MazeKruskalAlgo" should "generate a maze with correct dimensions and entrance/exit" in {
    val width = 4
    val height = 4
    val entrance = DefaultCell(0, 0)
    val exit = DefaultCell(3, 3)

    val mazeGenerator = new MazeKruskalAlgo()
